                        ...
                        <div>
                            ...
                            <div class="grid grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <p class="font-semibold text-gray-900">Rp {{ number_format($booking->unit_price, 0, ',', '.') }}</p>
                                </div>
                                <div>
                                ...
                                <div>
                                    ...
                                    <p class="font-semibold text-green-600">-Rp {{ number_format($booking->discount_amount, 0, ',', '.') }}</p>
                                </div>
                                @endif
                                <div>
                                    <p class="text-sm text-gray-500">Final Amount</p>
                                    <p class="text-xl font-bold text-purple-600">Rp {{ number_format($booking->final_amount, 0, ',', '.') }}</p>
                                </div>
                            </div>
                        </div>
                    </div>
                        ...
                        @if($booking->final_amount > 0)
                            <form action="{{ route('bookings.upload-proof', $booking) }}" method="POST" enctype="multipart/form-data" class="space-y-4">
                                ...
                                <div>
                                    ...
                                    <input type="text" name="payment_reference" placeholder="e.g., Transfer ref number"
                                           class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent">
                                </div>
                                <button type="submit" 
                    ...
                    @elseif($booking->status === 'paid')
                        ...
                        <a href="{{ route('tickets.show', $booking) }}" 
                           class="w-full bg-gradient-to-r from-green-600 to-teal-600 hover:from-green-700 hover:to-teal-700 text-white font-bold py-3 px-6 rounded-xl transition duration-300 flex items-center justify-center mb-4">
                            <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 5v2m0 4v2m0 4v2M5 5a2 2 0 00-2 2v3a1 1 0 001 1h1a1 1 0 011-1V8a1 1 0 01-1-1H4a1 1 0 00-1 1v6a1 1 0 001 1v1a2 2 0 002 2h14a2 2 0 002-2v-1a1 1 0 001-1V7a1 1 0 00-1-1h-1a1 1 0 01-1 1v2a1 1 0 01-1 1h-1a1 1 0 01-1-1V5a2 2 0 00-2-2H5z"></path>
                            </svg>
                            Lihat Tiket
                        </a>
                    ...
                    <div class="mt-6 pt-6 border-t border-gray-200">
                        ...
                        <div class="space-y-2">
                            ...
                            <div class="flex justify-between">
                                ...
                                <span class="text-sm font-semibold">{{ $booking->booking_date->format('d/m/Y H:i') }}</span>
                            </div>
                            @if($booking->payment_date)
                                <div class="flex justify-between">
                                    <span class="text-sm text-gray-500">Payment Date:</span>
                                    <span class="text-sm font-semibold">{{ $booking->payment_date->format('d/m/Y H:i') }}</span>
                                </div>
                            @endif
                            @if($booking->expired_at && $booking->status === 'pending')
                                <div class="flex justify-between">
                                    <span class="text-sm text-gray-500">Expires At:</span>
                                    <span class="text-sm font-semibold text-red-600">{{ $booking->expired_at->format('d/m/Y H:i') }}</span>
                                </div>
                            @endif
                        </div>
                    </div>
